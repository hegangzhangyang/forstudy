...
if __name__ == '__main__':
    ...
    df_budget = pd.read_excel("2020年5月工资标准表.xlsx").fillna(0)
    ...
    df_budget["挖潜增效奖"] = 0
    df_budget.loc[df_budget["职务"] == "部长", "挖潜增效奖"] = 220000
    df_budget.loc[df_budget["职务"] == "副部长", "挖潜增效奖"] = 190000
    df_budget.loc[df_budget["职务"] == "部长助理", "挖潜增效奖"] = 90000
    df_budget.loc[~df_budget["职务"].isin(["部长", "副部长", "部长助理"]), "挖潜增效奖"] = 1900 * df_budget["奖金系数"]

    df_budget["加班工资"] = df_budget.loc[
                            ~df_budget["职务"].isin(["部长", "副部长", "高级主管", "主管", "部长助理"]), "岗位工资"] / 21.75 * 3
    ...
    df_budget["应发工资"] = (df_budget["预支年薪"] + df_budget["岗位工资"] + df_budget["奖金"] + df_budget["加班工资"] + df_budget[
        "年功保留工资"] \
                         + df_budget["竞业津贴"] + df_budget["技术津贴"] + df_budget["回民补贴"] + df_budget["女工洗理费"] + df_budget[
                             "住房补贴"] \
                         + df_budget["工作补贴"] + df_budget["交通补贴"] + df_budget["班中餐"] + df_budget["误餐"] + df_budget[
                             "中班津贴"] + df_budget["夜班津贴"] \
                         + df_budget["其他补发"] + df_budget["其他补扣"] + df_budget["取暖费"] + df_budget["挖潜增效奖"]) * df_budget[
                            "合计"] + df_budget[
                            "异地津贴"] + df_budget["异地差旅费"] + df_budget["外租房津贴"]
    ...
    df_budget["残疾人保障金"] = 10520 * 12 * 1500 * 0.015 / df_budget["合计"].sum() * df_budget["合计"]
    ...
    df_budget.to_sql("bdget20205", conn, if_exists="replace")
    excelFormat("2020年5月份岗位预算表.xlsx", df_budget, 9, 15, "A:BU")
