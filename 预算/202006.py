...
if __name__ == '__main__':
    ...
    df_budget = pd.read_excel("2020年6月工资标准表.xlsx").fillna(0)
    ...
    df_budget["应发工资"] = (df_budget["预支年薪"] + df_budget["岗位工资"] + df_budget["奖金"] + df_budget["加班工资"] + df_budget[
        "年功保留工资"] \
                         ...
                         + df_budget["工作补贴"] + df_budget["交通补贴"] + df_budget["班中餐"] + df_budget["误餐"] + df_budget[
                             "中班津贴"] + df_budget["夜班津贴"] \
                         + df_budget["其他补发"] + df_budget["其他补扣"] + df_budget["取暖费"]) * df_budget["合计"] + df_budget[
                            "异地津贴"] \
    ...
    df_budget.to_sql("bdget20206", conn, if_exists="replace")
    excelFormat("2020年6月份岗位预算表.xlsx", df_budget, 9, 15, "A:BU")
